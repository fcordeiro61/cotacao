<x-app-layout>
    ...
    <div class="list-scroll">
                ...
                <form name=formCustomer method="POST" action="{{ route('customer.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="is_foreign" id="is_foreign"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            <option value="0"
                                {{ old('is_foreign', $customer->is_foreign ?? '') == 0 ? 'selected' : '' }}>
                                Não</option>
                            <option value="1"
                                {{ old('is_foreign', $customer->is_foreign ?? '') == 1 ? 'selected' : '' }}>
                                Sim</option>
                        </select>
                    </div>
        ...
        <div class="list-card">
            <div class="list-detail">
                ...
                <div class="form-group mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="street">Logradouro</label>
                ...
                <div class="form-group mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="complement">Complemento</label>
        ...
        <div class="list-card">
            <div class="list-detail">
                <div class="form-group mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="district">Bairro</label>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="state"  readonly
                        ...
                        @foreach($states as $uf => $nome)
                            <option value="{{ $uf }}"
                                {{ old('state', $customer->state ?? '') === $uf ? 'selected' : '' }}>
                                {{ $nome }}
                            </option>
                        @endforeach
                    </select>
    ...
    <script>
        ...
        function buscarCEP() {
            console.log('buscarCEP - inicio...');
            const form = document.forms['formCustomer'];
            const cep = form.zip_code.value.replace(/\D/g, '');
            //let cep = document.getElementById('zip_code').value.replace(/\D/g, '');
  ...
  .then(data => {
    ...
    // console.log('buscarCEP - logradouro: ', data.logradouro);
...
//     }
//     // preenche os campos normalmente
//     form.zip_code.value = data.logradouro;
//     document.getElementById('street').value = data.logradouro;
//   })
//   .catch(err => {
//     console.error('Erro ao buscar o CEP:', err);
